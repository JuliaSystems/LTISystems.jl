...
# zpk construction
s1m[1] = zpk([-2], [1, 1], 1, 1)
s2m[1] = zpk([-2], [1., 1.], 1, Float64(1))
s3m[1] = zpk(5.0, 1)
s4m[1] = zpk([1//2], [1, 1], 1, 1)
s5m[1] = zpk([1], [1], 3, 2)
...
@test zpk(zm, pm, km,1) ≈ mimo(reshape([s1m[1], s2m[1]], 2, 1))
@test zpk(5*eye(2), 1) ≈ mimo( [[s3m[1] zero(s3m[1])]; [zero(s3m[1]) s3m[1]]])
@test_throws DomainError zpk(zm, pm.', km,1)

# tf construction
s1m[2] = tf([1, 2], [1, 2, 1], 1)
s2m[2] = tf([1, 2], [1., 2., 1], Float64(1))
s3m[2] = tf(5.0, 1)
s4m[2] = tf([1//2, 3//5], [1, 1, 2], 1)
s5m[2] = tf([1], [1], 2)
...
@test tf(numm, denm, 1) ≈ mimo(reshape([s1m[2] s2m[2]],2,1))
@test tf(5*eye(2), 1) ≈ mimo( [[s3m[2] zero(s3m[2])]; [zero(s3m[2]) s3m[2]]])
@test_throws DomainError tf(numm, denm.', 1)
